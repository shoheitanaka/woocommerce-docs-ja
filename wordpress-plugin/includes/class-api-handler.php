 ...
 * API処理クラス
 * 
 * WooCommerce Docs Japanese のREST APIエンドポイントを提供
 */
...
class WC_Docs_JA_API_Handler {
    ...
    public function register_routes() {
        ...
        // 単一ドキュメント取得
        register_rest_route(self::NAMESPACE, '/docs/(?P<id>\d+)', array(
            'methods' => 'GET',
        ...
        // スラッグでドキュメント取得
        register_rest_route(self::NAMESPACE, '/docs/slug/(?P<slug>[a-z0-9-]+)', array(
            'methods' => 'GET',
        ...
        // ドキュメント検索
        register_rest_route(self::NAMESPACE, '/docs/search', array(
            'methods' => 'GET',
        ...
        // バージョン一覧取得
        register_rest_route(self::NAMESPACE, '/versions', array(
            'methods' => 'GET',
    ...
    public function get_docs($request) {
        ...
        // バージョンフィルター
        if (!empty($params['version']) && $params['version'] !== 'latest') {
    ...
    private function format_doc_response($post) {
        ...
        return array(
            ...
            'last_updated' => $updated ?: $post->post_modified
        );
    }
}
