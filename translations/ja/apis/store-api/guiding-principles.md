...
* 曖昧な用語は避け、プロパティ名は "WooCommerce "用語や設定名ではなく、理解しやすい言語を使用するようにします。
...
| `wc/store/v1/cart/items`｜カートアイテム｜カートアイテムオブジェクトのリストです。
|`wc/store/v1/products` | 商品 | 商品オブジェクトのリスト
|`wc/store/v1/products/1` | 商品 | 商品オブジェクト
...
顧客や注文データを含むリソースは、現在のセッションのみを反映する必要があります。プライバシーの侵害やセキュリティの問題になるので、他の顧客のデータを返さないでください。
...
APIから送られてくるデータを適切にエスケープするのはクライアントの責任だが、安全でない可能性のあるデータを返すのは避けるべきだ。

## リソースのコレクションはページ分割されるべきである

大量のデータは、サーバーに負担をかけないようにページ分割されるべきである。例えば、商品のコレクションを返す場合。

* レスポンスヘッダー `X-WP-Total`、 `X-WP-TotalPages`、 Link を使って、利用可能なリソースを示す。
* 特定のページを取得するには、パラメータ `page` と `per_page` を使用します。
* `per_page`の最大許容値は100です。
...
* `403 Forbidden` リクエストが許可されない場合、例えば提供されたセキュリティnonceが無効な場合。
* リソースが存在しない場合。
