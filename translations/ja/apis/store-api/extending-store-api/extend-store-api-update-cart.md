...
このサーバーサイドの処理によってカートの状態が変更され、クライアントサイドのカートまたはチェックアウトブロックに表示されるデータを更新したいとします。
...
__inline_code_0__：登録したコールバックを実行させたいことを通知し、コールバックにデータを渡すために使用する。唯一の引数としてオブジェクトを受け取ります。
