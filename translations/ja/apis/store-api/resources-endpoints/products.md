...
|`max_price` | string | no | 通貨の最小単位を使用して提供される最大価格に基づいて、結果セットを製品に制限します。例：100.25米ドルには10025を、1025円には1025を指定します。|
...
| `related` | integer | no | 特定のプロダクトIDに関連するプロダクトに結果を限定する。                                                                                                                                                                        |
...
| いいえ｜`after`｜親製品（バリエーション用）へのリンク
| `upsells` | アップセル商品へのリンク (設定されている場合)
| `cross_sells`|クロスセル商品取得へのリンク (設定されている場合) | はい｜｜`date_column`|クロスセル商品取得へのリンク (設定されている場合)
| はい｜`related`｜関連商品の取得リンク｜はい｜`date`｜関連商品の取得リンク(設定されている場合)
