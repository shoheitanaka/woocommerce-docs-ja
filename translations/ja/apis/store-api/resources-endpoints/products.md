...
| 属性 | タイプ | 必須 | 説明 |
| :------------------------------------------ | :------ | :------:| :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `search` | integer | no | 結果を文字列にマッチするものに限定する。                                                                                                                                                                                             |
| `slug`｜文字列｜no｜結果を特定のスラッグを持つ商品に限定する。カンマで区切る。                                                                                                                                                           |
...
| `parent` | array | no | 結果セットを特定の親IDのものに限定する。                                                                                                                                                                                   |
| `parent_exclude` | array | no | 結果セットを特定の親ID以外のすべてのアイテムに制限する。                                                                                                                                                                 |
...
| `brand_operator`｜文字列｜no｜商品ブランド用語を比較する演算子。許容される値：`in`、`not_in`、`and`。
| `_unstable_tax_[product-taxonomy]`          | string  |    no    | Limit result set to products assigned to the term ID of that custom product taxonomy.INLINE_CODE_43__は登録されたカスタム商品タクソノミーのキーでなければなりません。                                                               |
...
| `tag_operator` | string | no｜商品タグを比較する演算子。許可される値：`in`、`not_in`、`and`。
| `on_sale` | boolean | no | 結果をセール中の商品に限定する。                                                                                                                                                                                                 |
| `min_price` | string | no | 通貨の最小単位で指定された最低価格に基づいて、結果を商品に限定する。例：100.25米ドルには10025を、1025円には1025を指定する。|
...
| `attributes` | array | no | 結果を特定の属性に限定します。`attribute` (タクソノミー)、 `term_id` または `slug` を含むオブジェクトの配列と、 比較のためのオプション `operator` を期待します。                                                             |
| `attribute_relation`                        | string  |    no    | The logical relationship between attributes when filtering across multiple at once.                                                                                                                                                   |
| `catalog_visibility` | string | no | 非表示または表示されるカタログ商品を決定します。許可される値：許される値： `any`、 `visible`、 `catalog`、 `search`、 `hidden`。
| `rating` | boolean | no | 結果を特定の平均評価を持つ商品に限定する。                                                                                                                                                                           |
...
デフォルトでは、Store APIは商品のバリエーションを除外します。`type=variation`を使用すると、商品のバリエーションを取得できます。
