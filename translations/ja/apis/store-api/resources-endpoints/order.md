...
このエンドポイントには、`key`という必須パラメータが1つあります。ゲストオーダーの場合は、`billing_email`を追加する必要があります。

```sh
curl "https://example-store.com/wp-json/wc/store/v1/order/{ORDER_ID}?key={KEY}&billing_email={BILLING_EMAIL}"
```

オーダーオブジェクトの完全なレスポンスを返します（[オーダーレスポンス](#order-response)を参照）。

## レスポンス

オーダーエンドポイントは、`/cart`と同じ形式のレスポンスを返します。オーダーオブジェクトは、オーダーアイテム、適用されたクーポン、配送先住所と料金、および機密性のない顧客データを含みます。
...
オーダーアクションが実行できない場合、エラー応答が返されます。これには、理由コードとエラーメッセージが含まれます：
