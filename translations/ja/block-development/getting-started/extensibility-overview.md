...
| ドキュメント | ディスクリプション |
| ----------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| チェックアウトフローとイベント](/docs/block-development/extensible-blocks/cart-and-checkout-blocks/checkout-payment-methods/checkout-flow-and-events/)                            | チェックアウト・ブロックのチェックアウト・フローと、サブスクライブ可能な様々な発行イベントについて。|
...
以下の参考資料に加えて、[`block-checkout`パッケージのドキュメント](https://github.com/woocommerce/woocommerce/blob/trunk/plugins/woocommerce/client/blocks/packages/checkout/README.md)(`@woocommerce/dependency-extraction-webpack-plugin`パッケージのドキュメント)を参照してください。
