...
**エラーメッセージ**：エラーメッセージ**：ユーザーが実行したアクションが完了できなかった場合。(これにはバリデーションメッセージが含まれます。) ユーザーにデータ入力を要求する場合、各フィールドがフォーマット、範囲、フィールドが必須かどうかなどの要件を満たしているかどうかを確認してください。各フィールドに隣接してバリデーションメッセージを表示することで、ユーザーが各項目に対して適切な判断ができるようにします。専門用語は避けましょう。
