...
ミニファイされたCSSファイルを取得するには、リポジトリのルートディレクトリから`pnpm --filter='@woocommerce/classic-assets' build`を実行してください。ゼロから開発環境をセットアップするには、WooCommerce開発環境のセットアップガイドの[依存関係のインストールとアセットの生成方法](https://github.com/woocommerce/woocommerce/blob/trunk/DEVELOPMENT.md)のセクションを参照してください。
