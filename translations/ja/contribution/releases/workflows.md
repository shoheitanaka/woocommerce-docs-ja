...
| PRは`release/x.y`ブランチにマージされ、`cherry pick to trunk`ラベルが付けられます。コアロジックには[`shared-cherry-pick`](https://github.com/woocommerce/woocommerce/blob/trunk/.github/workflows/shared-cherry-pick.yml)を使用。|
...
| ワークフロー｜トリガー｜何をするのか？
| -------- | ------- | ------------ |
| [リリースPRにマイルストーンを自動追加](https://github.com/woocommerce/woocommerce/blob/trunk/.github/workflows/auto-milestone-release-prs.yml) | `release/x.y`ブランチでオープンまたは再開されたPRに、一致するマイルストーンを割り当てます。|
| [マージ時にマイルストーンを自動で割り当てる](https://github.com/woocommerce/woocommerce/blob/trunk/.github/workflows/pr-auto-milestone-on-merge.yml) | `trunk` で PR をクローズ | `trunk` にマージされた PR のうち、まだマイルストーンを持たないものにマイルストーンを割り当てます。|

### リリースイベントと検証

| ワークフロー｜トリガー｜何をするのか？
| -------- | ------- | ------------ |
| [Release: リリースイベントプロキシ](https://github.com/woocommerce/woocommerce/blob/trunk/.github/workflows/release-release-events-proxy.yml) | リリースの公開またはプレリリースを行う | [`release-new-release-published`](https://github.com/woocommerce/woocommerce/blob/trunk/.github/workflows/release-new-release-published.yml) にリリース後のアクションを委譲する: Slack通知の送信、安定版リリースのグローバル変更ログの更新、ベータ版リリースの *Generate Number of Commits and Contributors* を呼び出す。|
...
これらのワークフローは、リリースプロセス中にリリースリードによってトリガーされます。これらは、[ビルドとパブリッシングガイド](/docs/contribution/releases/building-and-publishing)やリリーストラッキングの問題で参照されているワークフローです。
...
| [Release: 安定版タグの更新](https://github.com/woocommerce/woocommerce/blob/trunk/.github/workflows/release-update-stable-tag.yml) | WordPress.orgの安定版タグを更新し、リポジトリに同期するPRを作成します。|
