...
1. **原因を特定する**：失敗したテストがtrunkでパスしているか確認してください。もし通っていれば、その修正はバックポートする必要があります。
2. **テストの修正をバックポートする**：可能であれば、関連するテスト修正をトランクからリリースブランチに [バックポート](/docs/contribution/releases/backporting) して、CI ワークフローを再実行します。
3. **複雑なケースに対処する**：依存関係のためにバックポートができない場合や、原因がはっきりしない場合は、発見したことを文書化し、リリースのSlackチャンネルで助けを求めてください。Heart of Gold - Flux」チームは、リリース作業を妨げるCI問題の解決を支援することができます。
...
[1.GitHubのドラフトリリースや不正なリリースのタグを削除してください：
   ...
   - コード > タグ**で、間違ったバージョンのタグを削除してください。これをスキップすると、最終的なリリースが履歴の間違ったコミットを指す可能性があります。
...
### 内部チェック/監視中に深刻なバグが検出されました {#deploy-serious-bug}.

RC および安定版リリースでは、リリースが一般公開される前に、ステージング環境へのデプロイとエラーの監視が必要です。この監視期間中に重大なバグが検出された場合は、以下の手順に従ってください：

1. **ステージング環境でのデプロイのリバート**を要求してください。
2. **リリース・プロセス** を直ちに一時停止し、追跡中の問題の残りのステップを続行しないでください。
3. **バグについての詳細を含め、リリースがブロックされたことを反映させるため、追跡中の問題**を更新してください。
4. **作成されたGitHubリリースのドラフトは**公開しないでください。これらは後で検証に合格したバージョンとともに公開されます。
5. **関連するエンジニアリングチーム**と調整し、修正を開発してください。
6. *リリースのスケジュールを調整したり、公に伝える必要がある場合は、** Developer Advocacy**を巻き込んでください([遅延については下記をお読みください](#release-delay))。

#### バグ修正がリリースブランチにマージされた後、どのように進めるか？

[1. **INLINE_CODE_4__](https://github.com/woocommerce/woocommerce/actions/workflows/release-create-tracking-issue.yml)ワークフローを実行して、新しいバージョンの新しい追跡課題**を作成してください(例: `-rc.1`中にバグが検出された場合は`-rc.2`、`x.y.1`の監視中にバグが検出された場合は`x.y.2`)。既存の追跡issueを再利用しないでください。
2. **新バージョンのリリース手順**に通常通り従ってください。
3. **影響を受けるバージョン・シリーズのすべてのドラフト・リリース**を公開してください。以前のバージョンが一般公開されていない場合でも、有効なバージョンと一緒に公開しなければなりません。各バージョンには、それぞれの変更履歴セクションがあります。
...
### リリースを延期する必要がある。どうしますか？リリース遅延} {#release-delay

1.社内にSlackスレッドを作成し、エンジニアリングチームやDev Advocacyとコミュニケーションをとる。これは、チームが追加的なコンテキストを共有し、スケジュールの変更を確認したり、異議を唱えたりする機会にもなる。
2.Dev Advocacyに遅延を公に伝えるよう依頼する。
3.修正パッチのリリースに明確なETAがある場合、[リリースカレンダー](https://developer.woocommerce.com/release-calendar/)を新しい日付で更新する。
